...
const submitBtn = document.querySelector('button.accent-btn');
submitBtn.addEventListener('click', formValidity);
...
function readMoreBtn(e) {
  if (e.target.classList.value === 'btn') {
    const parent = e.target.parentElement
    const spanHidden = parent.querySelector('p').querySelector('span.hidden')
    const spanNotHidden = parent.querySelector('p').querySelector('span:not(.hidden)')
    spanHidden.classList.remove("hidden")
    spanNotHidden.classList.add("hidden")
...
articleWrapper.addEventListener('click', deleteBtn)
articleWrapper.addEventListener('click', readMoreBtn)
